...
1. `简约便签` / `minimal-note`
2. `柔粉信笺` / `gentle-letter`（持久化值：`温柔信纸`）
3. `雾紫诗笺` / `poetic-card`（持久化值：`诗意卡片`）
4. `日光便签` / `vintage-receipt`（持久化值：`复古收据`）

`Gift.theme` 继续保存既有中文值，以兼容当前 mock 和已创建数据；theme id 仅用于集中视觉配置。主题会影响创建端选择卡和预览、接收端封面态、正文态和接收完成态的色板、圆角、顶部标签、引用块、主按钮与轻点缀。默认主题和 mock gift 均为 `简约便签`；`vintage-receipt` 仅保留技术 id，不再展示收据语义。
