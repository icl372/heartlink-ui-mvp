...
4. `src/app/lib/giftUrl.ts` owns the current `/to/:token` route prefix and shared-link origin selection: `VITE_APP_BASE_URL`, then `VITE_SITE_URL`, then `VITE_PUBLIC_SITE_URL`; stale Vercel deployment origins fall back to `https://www.xygift.cn`, while local development uses the current browser origin.
...
6. Explicit mock tokens retain local-only writes. For real tokens with `VITE_USE_SUPABASE=true`, TODO-039 records one browser-deduplicated open and an idempotent accepted state through the owned status endpoint.
